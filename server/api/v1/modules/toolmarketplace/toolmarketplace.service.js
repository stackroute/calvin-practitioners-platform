...
function checkIfToolExists(toolid, done) {
   ...
   client.execute(checkQry, (err, result) => {        if (err) {
           console.log('err', err);
...
function AddToolinMarketplace(tool, done) {    console.log('toolid', tool.toolid);
   console.log("inside addtoolmarketplkace service");    if (checkIfToolExists(tool.toolid, (error, toolExists) => {
       if (toolExists) {
           ...
           return done(null, 'Tool Already exist');        } else {            addToolinDB(tool, (err) => {
               if (err) {
