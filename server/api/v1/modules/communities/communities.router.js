...
router.post('/:domainName', (req, res) => {
  try {
    communityCtrl.postNewcommunityDetails(req.params.domainName, req.body, (err, results) => {
      if (err) {
        return res.status(500).send({ error: 'Unexpected errors occurred in creating a new community, try again later' });
      }
      return res.status(200).send(results);
    });
  } catch (err) {
    console.log("Unexpected exception in creating a new community ", err);
    return res.status(500).send({ error: 'Unexpected error occurred, try again later' });
  }
});
